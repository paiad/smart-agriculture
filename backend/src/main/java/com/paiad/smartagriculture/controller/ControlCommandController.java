...
public class ControlCommandController {
    ...
    public ApiResult<ControlCommandVO> getStatus(@PathVariable String requestId) {
        ...
        if (cmd.getStatus() == CommandConstants.STATUS_SENT && cmd.getSentAt() != null) {
            LocalDateTime timeout = cmd.getSentAt().plusSeconds(CommandConstants.COMMAND_TIMEOUT_SECONDS);
            if (LocalDateTime.now().isAfter(timeout)) {
                cmd.setStatus(CommandConstants.STATUS_TIMEOUT);
